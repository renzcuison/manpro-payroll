...
const SuperApplicationsChooseModal = ({ open, close, teamID }) => {
    const navigate = useNavigate();

    const handleViewRequest = (id) => {
        navigate('/hr/applications?employeeID=' + id)
    }
    const handleViewList = (id) => {
        navigate('/hr/applications-list?employeeID=' + id)
    }
            ...
            <Dialog sx={{
                ...
                <DialogContent>
                    <Stack sx={{ margin: '0 auto', width: '80%', pb: 3 }} spacing={3}>
                        <Button variant='contained' sx={{ p: 2, backgroundColor: '#3286d7' }} onClick={() => handleViewRequest(teamID)}>
                            Applications Request
                        </Button>
                        <Button variant='contained' sx={{ p: 2, backgroundColor: '#7eb73d' }} onClick={() => handleViewList(teamID)}>
                            Applications List
                        </Button>
                    </Stack>
                </DialogContent>
...
export default SuperApplicationsChooseModal
