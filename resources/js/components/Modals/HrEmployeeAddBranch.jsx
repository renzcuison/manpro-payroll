...
const HrEmployeeAddBranch = ({ open, close }) => {
    ...
    const handleSubmitBranch = (e) => {
        ...
        if (!/^[a-zA-Z0-9\s]+$/.test(addBranch)) {
            setTitleError('Only alphanumeric characters are allowed.');
            return;
        }

        const formData = new FormData();
        formData.append('branch_name', addBranch);
        ...
        new Swal({
            ...
            if (res.isConfirmed) {
                axiosInstance.post('/add-branch', formData, { headers })
                    .then((response) => {
    ...
    const handleDeleteBranch = (id) => {
        new Swal({
            ...
            if (res.isConfirmed) {
                axiosInstance.put(`/delete-branch/${id}`, { is_deleted: '1' }, { headers })
                    .then((response) => {
                        location.reload();
                        forceUpdate();
                    })
        ...
        <>
            <Dialog sx={{
                ...
                <Box className="d-flex justify-content-between" >
                    <DialogTitle>
                        <Typography className="text-center">Add new Branch</Typography>
                    </DialogTitle>
                        ...
                        >
                            <FormGroup row={true} className="d-flex justify-content-between" sx={{
                                ...
                                <FormControl sx={{
                                    ...
                                    error={!!titleError}>
                                    <InputLabel id="demo-simple-select-label" shrink={true} sx={{ backgroundColor: 'white', paddingLeft: 1, paddingRight: 1, borderColor: '#97a5ba' }}>Branch Name</InputLabel>
                                ...
                                {branchData.map((res, index) => {
                                    return (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell colSpan={2}> {res.branch_name}</TableCell>
                                            <TableCell className='text-center'>
                                                <Button
                                                    type="submit"
                                                    variant="text"
                                                    style={{ color: 'red' }}
                                                    onClick={() => handleDeleteBranch(res.branch_id)}
                                                >
...
export default HrEmployeeAddBranch
