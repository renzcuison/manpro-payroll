...
const EmployeeTaxes = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        axiosInstance.get(`/additional_benefits/${4}/${empID}`, { headers }).then((response) => {
    ...
    function bracketList(id) {
        return bracketData
            ...
            .map((bracket, index) => (
                <div key={index}>
                    {`${bracket.rangeFrom ? bracket.rangeFrom.toString() : 0} TO ${bracket.rangeTo ? bracket.rangeTo.toString() : 0} = ${bracket.share ? bracket.share.toString() + '%' : 0 + '%'} = ${bracket.shareAmount ? bracket.shareAmount.toString() : 0}`}
                </div>
            ));
    }
            ...
            <div>
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={listData.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} sx={{ '.MuiTablePagination-selectLabel': { paddingTop: '18px' }, '.MuiTablePagination-displayedRows': { paddingTop: '18px' } }} />

                    </>
                )}

            </div>
            <HrEmployeeAdditionalBenefitModal open={openModal} close={handleCloseModal} type={4} benefitData={null} />
        </>
    )
}
