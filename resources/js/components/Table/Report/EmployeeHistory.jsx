...
const EmployeeHistory = () => {
    ...
    const getEmployee = async (month_val, year_val) => {
        ...
        await axiosInstance.get(`/employeesHistory/${empID}/${dates.join(',')}`, { headers })
            .then((response) => {
    ...
    const handleToPrint = useReactToPrint({
        ...
        documentTitle: 'Employee_History_Data',
        onAfterPrint: () => {
            Swal.fire({
                                ...
                                .map((emp, index) => {
                                    return (
                                        <TableRow key={index} hover role="checkbox" tabIndex={-1}>
                                            ...
                                            <TableCell>{moment(emp.date_created).format('MMM. D, YYYY')}</TableCell>
                                            <TableCell>{emp.date_hired ? moment(emp.date_hired).format('MMM. D, YYYY') : 'N/A'}</TableCell>
                                        </TableRow>
                                    )
                                })}
                            {emptyRows > 0 && (
