...
const SalaryIncrease = () => {
    ...
    const getEmployee = async (month_val, year_val) => {
        ...
        await axiosInstance.get(`/salary-increase/${empID}/${dates.join(',')}`, { headers })
            .then((response) => {
    ...
    const handleFilter = (event) => {
        const filtered = employeeDetails.filter(employee => `${employee?.lname} ${employee?.fname} ${employee?.mname}`.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()));
    ...
    const handleToPrint = useReactToPrint({
        ...
        documentTitle: 'Employee_Salary_Increase_Data',
        onAfterPrint: () => {
            Swal.fire({
                                    ...
                                    .map((emp, index) => {
                                        return (
                                            <TableRow key={index} hover role="checkbox" tabIndex={-1}>
                                                ...
                                                <TableCell>Php {emp.from_rate}</TableCell>
                                                <TableCell>Php {emp.new_rate}</TableCell>
                                                <TableCell>Php {emp.new_rate - emp.from_rate}</TableCell>
                                                <TableCell>{moment(emp.created_at).format('MMM. D, YYYY')}</TableCell>
                                            </TableRow>
                                        )
                                    })}
                                {emptyRows > 0 && (
