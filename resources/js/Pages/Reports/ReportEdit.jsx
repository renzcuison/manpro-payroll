...
const ReportEdit = () => {
        ...
        if (id){
            ...
            axiosInstance.get(`/getReport`, { params: data, headers })
                .then((response) => {
                    ...
                    setDate(dayjs(response.data.report.date)); 

                    setType(response.data.reportType.id);
                    setPeriodFrom(dayjs(response.data.report.period_from)); 
                    setPeriodTo(dayjs(response.data.report.period_to)); 
                    
                    setDescription(response.data.report.description);
                    setAttachment(response.data.report.attachment);

                    setAttachment(response.data.report.attachment.replace('reports/', ''));

                    const assignedEmployees = response.data.selectedEmployees.map(emp => emp.user_id);
    ...
    const checkInput = (event) => {
        ...
        if ( !title || !date || !type || !periodFrom || !periodTo || !description || !attachment || selectedEmployees.length == 0 ) {
            Swal.fire({
    ...
    const saveEdit = (event) => {
        ...
        formData.append('attachmentChanged', attachmentChanged);
        ...
        axiosInstance.post('/editReport', formData, { headers })
            .then(response => {
                ...
                if (response.data.status === 200) {
                    Swal.fire({
                        ...
                        navigate(`/report-view/${response.data.report}`);
                    });
                }
            })
            .catch(error => {
                        ...
                        <>
                            <Box component="form" sx={{ mx: 6, mt: 3, mb: 6 }} onSubmit={checkInput} noValidate autoComplete="off" encType="multipart/form-data">
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3, mb: 6 }}>
                                    <Typography variant="h4" sx={{ fontWeight: 'bold' }}>Edit Document</Typography>
                                </Box>
                                ...
                                <FormGroup row={true} sx={{ '& label.Mui-focused': { color: '#97a5ba' } }}>
                                    <FormControl error={selectedEmployeesError} required sx={{ marginBottom: 3, width: '100%' }}>
                                    <InputLabel id="selectedEmployeeLabel">Assigned Employee</InputLabel>
                                        <Select
                                            ...
                                            input={<OutlinedInput label="Assigned Employee" />}
                                        ...
                                        >
                                            {employees.map((employee) => (
                                                ...
                                                >
                                                    {`${employee.fname} ${employee.mname || ''} ${employee.lname}`.trim()}
                                                </MenuItem>
                                            ))}
                                        </Select>
                                    </FormControl>
