...
const HMOAddModal = ({ open, close, onAddRow, refreshPlans }) => {
    ...
    const [HMOCompanyId, setHMOCompanyId] = useState("");
    ...
    const fetchCompanies = async () => {
        try {
            const res = await axiosInstance.get('/medicalRecords/getHMOCompanies', {
            ...
            if (companyList.length > 0) {
                setHMOCompanyId(companyList[0].id);
...
const handleSave = async () => {
  ...
  try {
    await axiosInstance.post("/medicalRecords/saveHMOPlans", payload, {
      headers: { Authorization: `Bearer ${user.token}` },
    });
    ...
    Swal.fire({
      icon: "success",
      title: "Success",
      text: "HMO Plan saved successfully!",
      timer: 2000,
            ...
            <Dialog open={open} fullWidth maxWidth="md"PaperProps={{ style: { padding: '16px', backgroundColor: '#f8f9fa', boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px', borderRadius: '20px', minWidth: '800px', maxWidth: '1000px', marginBottom: '5%' }}}>
                <DialogTitle sx={{ padding: 4, paddingBottom: 1 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h4" sx={{ marginLeft: 1 ,fontWeight: 'bold' }}> Add HMO Plan</Typography>
                    ...
                    <Box component="form" sx={{ mt: 3, my: 6 }} noValidate autoComplete="off" encType="multipart/form-data">
                        ...
                        <FormGroup 
                            ...
                            <FormControl sx={{ marginBottom: 3, width: '30%', '& label.Mui-focused': { color: '#97a5ba' },
                                ...
                                <FormControl>
                                    <InputLabel id="group-life-company-label">HMO Company</InputLabel>
                                    <Select
                                        labelId="group-life-company-label"
                                        label="HMO Company"
                                        value={HMOCompanyId}
                                        onChange={(e) => setHMOCompanyId(e.target.value)}
                                        >
