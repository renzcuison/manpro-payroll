...
const HMOAddCompanyModal = ({ open, close, onAddCompany }) => {
    ...
    const handleAdd = async () => {
        ...
        try {
            const response = await axiosInstance.post('/medicalRecords/saveHMOCompanies',
    ...
    axiosInstance.put(`/medicalRecords/editHMOCompany/${selectedCompanyId}`, {
        name: newCompanyName,
    }, {
        headers: { Authorization: `Bearer ${user.token}` }
    })
    ...
    const handleDeleteCompany = (companyId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axiosInstance.delete(`/medicalRecords/deleteHMOCompany/${companyId}`, {
                        headers: { Authorization: `Bearer ${user.token}` }
                    });
            ...
            <Dialog open={open} fullWidth maxWidth="lg" PaperProps={{ style: { padding: '16px', backgroundColor: '#f8f9fa', boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px', borderRadius: '20px', marginBottom: '5%' }}}>
                <DialogTitle sx={{ padding: 3, paddingBottom: 1, mt:2 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h4" sx={{ marginLeft: 1 ,fontWeight: 'bold' }}>  {isRenaming ? 'Edit HMO Company' : 'Add HMO Company'}</Typography>
...
export default HMOAddCompanyModal;
