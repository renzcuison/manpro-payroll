            ...
            >
                <DialogTitle sx={{ padding: 2, paddingBottom: 3 }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Typography variant="h4" sx={{ marginLeft: 1, fontWeight: "bold" }}>{`${logsView ? "Leave Credit Logs" : "Leave Credit Details"}`}</Typography>
                                        ...
                                        <TableBody>
                                            {leaveCreditLogs.length > 0 ? (
                                                leaveCreditLogs.map((log, index) => (
                                                    <TableRow key={index}>
                                                        <TableCell align="left"> <Typography>{dayjs(log.created_at).format('YYYY-MM-DD HH:mm:ss')}</Typography> </TableCell>
                                                        <TableCell align="left"> <Typography>{`${log.username} ${log.action}`}</Typography> </TableCell>
                                                    </TableRow>
                                                ))
                                            ) : (
                                                <TableRow>
                                                    <TableCell colSpan={5} align="center" sx={{ color: "text.secondary", p: 1 }}> No Leave Credits Found </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
                                    </Table>
                                        ...
                                        <TableBody>
                                            {leaveCredits.length > 0 ? (
                                                leaveCredits.map((leave, index) => {
                                                    ...
                                                    return (
                                                        <TableRow key={index}>
                                                            <TableCell> <Typography>{leave.app_type_name}</Typography> </TableCell>
                                                            <TableCell align="center"> <Typography>{leave.credit_number}</Typography> </TableCell>
                                                            <TableCell align="center"> <Typography>{leave.credit_used}</Typography> </TableCell>
                                                            <TableCell align="center" sx={{ color: remainingEmpty ? "#f44336" : remainingWarning ? "#e9ae20" : null }}> <Typography> {remainingCredits} </Typography> </TableCell>
                                                            <TableCell align="center"> <IconButton size="small" onClick={() => handleOpenEditLeaveCredit(leave)}> <Edit size="small" /> </IconButton> </TableCell>
                                                        </TableRow>
                                                    );
                                                })
                                            ) : (
                ...
                {openEditLeaveCredit && ( <LeaveCreditEdit open={openEditLeaveCredit} close={handleCloseEditLeaveCredit} leaveData={leaveData} /> )}
                {openAddLeaveCredit && ( <LeaveCreditAdd open={openAddLeaveCredit} close={handleCloseAddLeaveCredit} empId={userName} /> )}
