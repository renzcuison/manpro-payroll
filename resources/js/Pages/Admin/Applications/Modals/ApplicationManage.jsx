...
const ApplicationManage = ({ open, close, appDetails }) => {
    ...
    const handleFileDownload = async (filename, id) => {
        try {
            const response = await axiosInstance.get(`/applications/downloadAttachment/${id}`,{ responseType: "blob", headers, });
    ...
    return (
        <>
            <Dialog
                ...
                maxWidth="md"
                PaperProps={{
                    style: { backgroundColor: '#f8f9fa', boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px', borderRadius: '20px', minWidth: { xs: "100%", sm: "850px" }, maxWidth: '900px', marginBottom: '5%' }
                }}>
