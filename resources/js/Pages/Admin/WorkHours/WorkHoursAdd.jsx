...
const WorkHoursAdd = () => {
    ...
    const [workHourNameError, setWorkHourNameError] = useState(false);
    ...
    const checkInput = (event) => {
        ...
        if (!workHourName) {
            setWorkHourNameError(true);
        } else {
            setWorkHourNameError(false);
        }

        if ( workHourType == 'regular' ) {
            checkInputRegular(event);
        }

        if ( workHourType == 'split' ) {
            ...
            <Box sx={{ mx: 10, pt: 12 }}>
                <div className='px-4 block-content bg-light' style={{ boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px', borderRadius: '20px', minWidth: '800px', maxWidth: '1000px', marginBottom: '5%' }}>
                    <Box component="form" sx={{ mx: 6, mt: 3, mb: 6 }} onSubmit={checkInput} noValidate autoComplete="off" encType="multipart/form-data" >
                        ...
                        <Typography variant="h4" sx={{ mt: 3, mb: 6, fontWeight: 'bold' }}>Add Work Hour</Typography>
                        ...
                        <FormGroup row={true} className="d-flex justify-content-between" sx={{
                            ...
                            <FormControl sx={{ marginBottom: 3, width: '32%', '& label.Mui-focused': { color: '#97a5ba' },
                                ...
                                <TextField
                                    ...
                                    required
                                    id="workHourType"
                                    label="Work Hour Type"
                                    value={workHourType}
                                    onChange={(event) => handleWorkHourTypeChange(event.target.value)}
                                >
