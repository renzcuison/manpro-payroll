...
const ApplicationTypesEdit = ({ open, close, appTypeInfo }) => {
    const navigate = useNavigate();
    const storedUser = localStorage.getItem("nasya_user");
    const headers = getJWTHeader(JSON.parse(storedUser));

    const [name, setName] = useState(appTypeInfo.name);
    const [tenureship, setTenureship] = useState(appTypeInfo.tenureship_required);
    const [requireFiles, setRequireFiles] = useState(appTypeInfo.require_files);
            ...
            <Dialog
                ...
                <DialogTitle sx={{ padding: 4, paddingBottom: 1 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h4" sx={{ marginLeft: 1, fontWeight: 'bold' }}> Edit Application Type </Typography>
                    ...
                    <Box component="form" sx={{ mt: 3, my: 3 }} onSubmit={checkInput} noValidate autoComplete="off" encType="multipart/form-data" >
                        ...
                        <Box display="flex" justifyContent="center" sx={{ marginTop: '20px' }}>
                            <Button type="submit" variant="contained" sx={{ backgroundColor: '#177604', color: 'white' }} className="m-1">
                                <p className='m-0'><i className="fa fa-floppy-o mr-2 mt-1"></i> Update Application Type </p>
                            </Button>
                        </Box>
...
export default ApplicationTypesEdit;
