...
const JobTitlesEdit = ({ open, close, jobTitleInfo }) => {
    const navigate = useNavigate();
    const storedUser = localStorage.getItem("nasya_user");
    const headers = getJWTHeader(JSON.parse(storedUser));

    const [name, setName] = useState(jobTitleInfo.name);
    const [acronym, setAcronym] = useState(jobTitleInfo.acronym);
    const [status, setStatus] = useState(jobTitleInfo.status);
    ...
    const checkInput = (event) => {
        ...
        if (!name || !acronym || !status) {
            ...
            Swal.fire({
                customClass: { container: "my-swal" },
                title: "Are you sure?",
                text: "This job title will be updated",
                icon: "warning",
                showConfirmButton: true,
            ...
            <Dialog open={open} fullWidth maxWidth="md" PaperProps={{ style: { padding: '16px', backgroundColor: '#f8f9fa', boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px', borderRadius: '20px', minWidth: '800px', maxWidth: '1000px', marginBottom: '5%' } }}>
                <DialogTitle sx={{ padding: 4, paddingBottom: 1 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h4" sx={{ marginLeft: 1, fontWeight: 'bold' }}> Edit Job Title </Typography>
                    ...
                    <Box component="form" sx={{ mt: 3, my: 6 }} onSubmit={checkInput} noValidate autoComplete="off" encType="multipart/form-data" >
                        ...
                        <Box display="flex" justifyContent="center" sx={{ marginTop: '20px' }}>
                            <Button type="submit" variant="contained" sx={{ backgroundColor: '#177604', color: 'white' }} className="m-1">
                                <p className='m-0'><i className="fa fa-floppy-o mr-2 mt-1"></i> Update Job Title </p>
                            </Button>
                        </Box>
