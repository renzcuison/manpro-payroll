...
class EvaluationResponseController extends Controller
{
    ...
    public function getEvaluationResponse(Request $request)
    {
        ...
        try {
            ...
            $evaluatee = UsersModel::find($evaluationResponse->evaluatee_id);
            $evaluator = UsersModel::find($evaluationResponse->evaluator_id);
            $primary = UsersModel::find($evaluationResponse->primary_commentor_id);
            $secondary = UsersModel::find($evaluationResponse->secondary_commentor_id);

            $datetime = $evaluationResponse->updated_at
                ? date('M d, Y - h:i A', strtotime($evaluationResponse->updated_at))
                : null;
            $period_start_date = $evaluationResponse->period_start_at
                ? date('M d, Y', strtotime($evaluationResponse->period_start_at))
                : null;
            $period_end_date = $evaluationResponse->period_end_at
                ? date('M d, Y', strtotime($evaluationResponse->period_end_at))
                : null;
                    ...
                    $query->whereNull('deleted_at')
                        ...
                        ->with(['subcategories' => function ($q) {
                            $q->whereNull('deleted_at')
                                ...
                                ->with([
                                    'options' => function ($oq) {
                                        $oq->whereNull('deleted_at')->orderBy('order');
                                    }
                                ]);
                        }]);
                }
            ])->find($evaluationResponse->form_id);
            ...
            return response()->json([
                ...
                'evaluationResponse' => [
                    ...
                    'datetime' => $datetime,
                    'evaluatee_id' => $evaluatee ? $evaluatee->id : null,
                    'evaluatee_last_name' => $evaluatee ? $evaluatee->last_name : null,
                    'evaluatee_first_name' => $evaluatee ? $evaluatee->first_name : null,
                    'evaluatee_middle_name' => $evaluatee ? $evaluatee->middle_name : null,
                    'evaluator_id' => $evaluator ? $evaluator->id : null,
                    'evaluator_last_name' => $evaluator ? $evaluator->last_name : null,
                    'evaluator_first_name' => $evaluator ? $evaluator->first_name : null,
                    'evaluator_middle_name' => $evaluator ? $evaluator->middle_name : null,
                    'primary_commentor_id' => $primary ? $primary->id : null,
                    'primary_last_name' => $primary ? $primary->last_name : null,
                    'primary_first_name' => $primary ? $primary->first_name : null,
                    'primary_middle_name' => $primary ? $primary->middle_name : null,
                    'secondary_commentor_id' => $secondary ? $secondary->id : null,
                    'secondary_last_name' => $secondary ? $secondary->last_name : null,
                    'secondary_first_name' => $secondary ? $secondary->first_name : null,
                    'secondary_middle_name' => $secondary ? $secondary->middle_name : null,
                    'period_start_date' => $period_start_date,
                    'period_end_date' => $period_end_date,
                    'signature_filepath' => $evaluationResponse->signature_filepath,
    ...
    public function getEvaluationResponses(Request $request)
{
    ...
    try {
        ...
        $evaluationResponses = EvaluationResponse
            ...
            ->select(
                ...
                DB::raw("date_format(evaluation_responses.updated_at, '%b %d, %Y') as date"),
                'evaluatees.id as evaluatee_id', 'evaluatees.last_name as last_name',
                'evaluatees.first_name as first_name', 'evaluatees.middle_name as middle_name',
                'departments.id as department_id', 'departments.name as department_name',
                'branches.id as branch_id', 'branches.name as branch_name',
                ...
                'evaluation_responses.created_at',
                'evaluation_responses.updated_at',
                'evaluation_responses.status',
                'evaluation_responses.current_step'
            );

        // --- WORKFLOW FILTER: Only show evaluations for the user's current step ---
        if ($userID) {
            $evaluationResponses = $evaluationResponses->where(function ($query) use ($userID) {
                $query
                    ->where(function ($q) use ($userID) {
                        $q->where('evaluation_responses.evaluator_id', $userID)
                          ->where('evaluation_responses.current_step', 'evaluator');
                    })
                    ->orWhere(function ($q) use ($userID) {
                        $q->where('evaluation_responses.primary_commentor_id', $userID)
                          ->where('evaluation_responses.current_step', 'first_commentor');
                    })
                    ->orWhere(function ($q) use ($userID) {
                        $q->where('evaluation_responses.secondary_commentor_id', $userID)
                          ->where('evaluation_responses.current_step', 'second_commentor');
                    })
                    ->orWhere(function ($q) use ($userID) {
                        $q->where('evaluation_responses.evaluatee_id', $userID)
                          ->where('evaluation_responses.current_step', 'evaluatee');
                    });
            });
        }
        ...
        if ($request->search) {
            ...
            $evaluationResponses = $evaluationResponses->where(function ($query) use ($search) {
                $query->where(DB::raw("date_format(evaluation_responses.updated_at, '%b %d, %Y')"), 'LIKE', "%$search%")
                    ->orWhere('evaluation_forms.name', 'LIKE', "%$search%")
                    ->orWhere('evaluatees.last_name', 'LIKE', "%$search%")
                    ->orWhere('evaluatees.first_name', 'LIKE', "%$search%")
                    ->orWhere('evaluatees.middle_name', 'LIKE', "%$search%")
        ...
        foreach ($order_by as $order_param) {
            $key = $order_param['key'] ?? '';
            $sort_order = $order_param['sort_order'] ?? 'asc';
            if (!in_array($sort_order, ['asc', 'desc'])) $sort_order = 'asc';
            switch ($key) {
                case 'branch_name':
                    $evaluationResponses = $evaluationResponses->orderBy('branches.name', $sort_order);
                    break;
                case 'department_name':
                    $evaluationResponses = $evaluationResponses->orderBy('departments.name', $sort_order);
                    break;
                case 'last_name':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluatees.last_name', $sort_order);
                    break;
                case 'first_name':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluatees.first_name', $sort_order);
                    break;
                case 'middle_name':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluatees.middle_name', $sort_order);
                    break;
                case 'updated_at':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluation_responses.updated_at', $sort_order);
                    break;
                case 'form_name':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluation_forms.name', $sort_order);
                    break;
                case 'status':
                    $evaluationResponses = $evaluationResponses->orderBy('evaluation_responses.status', $sort_order);
                    break;
                default:
                    // ignore invalid keys
                    break;
        ...
        $skip = ($page - 1) * $limit;
        $evaluationResponses = $evaluationResponses->skip($skip)->take($limit)->get();
        $pageResponseCount = $evaluationResponses->count();
    ...
    //             'status' => 'pending', // or 'for_evaluator'
    ...
    public function saveEvaluationOptionAnswer(Request $request)
{
    ...
    try {
        ...
        switch ($subcategory->subcategory_type) {
            ...
            case "short_answer":
                return response()->json([
                    ...
                    'evaluationFormSubcategoryID' => $subcategory->subcategory_id
                ]);
            case "checkbox":
                ...
                if ($existingOptionAnswer) return response()->json([
                    ...
                    'evaluationOptionAnswer' => [
                        'response_id' => $existingOptionAnswer->response_id,
                        'option_id' => $existingOptionAnswer->option_id,
                    ]
                ]);
                break;
            case "dropdown":
            case "multiple_choice":
                // Only one option answer per subcategory and response
                $existingOptionAnswer = EvaluationOptionAnswer
                    ::join('evaluation_form_subcategory_options', 'evaluation_form_subcategory_options.id', '=', 'evaluation_option_answers.option_id')
                    ->where('evaluation_option_answers.response_id', '=', $request->response_id)
                    ->where('evaluation_form_subcategory_options.subcategory_id', '=', $subcategory->subcategory_id)
                    ->select('evaluation_option_answers.response_id', 'evaluation_option_answers.option_id')
                    ->first();
                if ($existingOptionAnswer) return response()->json([
                    'status' => 409,
        ...
        return response()->json([
            'status' => 201,
            'evaluationOptionAnswer' => [
                'response_id' => $newEvaluationOptionAnswer->response_id,
                'option_id' => $newEvaluationOptionAnswer->option_id
            ],
        ...
        DB::rollBack();
        Log::error('Error saving evaluation option answer: ' . $e->getMessage());
        return response()->json([
            'status' => 500,
            'message' => 'Server error: ' . $e->getMessage(),
        ], 500);
