...
public function createPeme(Request $request)
{
    ...
    if (!$user || !$user->client_id) {
        return response()->json(['message' => 'Client ID not found for user.'], 400);
    }
    ...
    if ($exists) {
        return response()->json(['message' => 'Duplicate record exists.'], 409); // Conflict
    }
    
    $peme = Peme::create($validatedData);

    return response()->json(['message' => 'Exam created successfully.', 'peme' => $peme], 201);
}
...
public function getPemeList()
{
    ...
    $pemeList = Peme::select('created_at', 'name')
                ...
                ->map(function ($peme) {
                    return [
                        'date' => Carbon::parse($peme->created_at)->format('F d, Y'), // e.g., May 25, 2025
                        'name' => $peme->name,
                    ];
                });
