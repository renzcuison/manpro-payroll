...
class AdminDashboardController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        // get employees
        $employees = UsersModel::with([
            'media',
            'department',
            'branch',
            'jobTitle',
            'latestAttendanceLog',
            'attendanceLogs' => function ($qry) {
                $qry->whereDate(
                    'timestamp',
        ...
        $attendances = AttendanceLogsModel::whereHas('user', function (
            $query
        ) use ($clientId) {
            $query
                ->where('client_id', $clientId)
        ...
        $milestones = Milestone::with(['user.media', 'comments.user'])
    ...
    public function getAttendanceToday1(Request $request)
    {
        ...
        $attendanceLogs = AttendanceLogsModel::whereHas('user', function (
            $query
        ) use ($clientId) {
            $query
                ->where('client_id', $clientId)
        ...
        $onLeaveUserIds = ApplicationsModel::whereHas('user', function (
            $query
        ) use ($clientId) {
            $query
                ->where('client_id', $clientId)
        ...
        foreach ($allEmployees as $employee) {
            ...
            if ($logs && $logs->isNotEmpty()) {
                ...
                if ($workHours->shift_type == "Regular") {
                    ...
                    $firstOut = Carbon::parse(
                        "$date {$workHours->first_time_out}"
                    );
                    $secondIn = Carbon::parse(
                        "$date {$workHours->second_time_in}"
                    );
                    $secondOut = Carbon::parse(
                        "$date {$workHours->second_time_out}"
                    );

                    $firstTimeIn = $logs->first(function ($log) use (
                        $firstOut
                    ) {
                        return $log->action === 'Duty In' &&
                            Carbon::parse($log->timestamp)->lt($firstOut);
                    });

                    $secondTimeIn = $logs->first(function ($log) use (
                        $firstOut,
                        $secondOut
                    ) {
                        return $log->action === 'Duty In' &&
                ...
                if (
                    $firstAttendance &&
                    Carbon::parse($firstAttendance->timestamp)->gt(
                        Carbon::parse("$date {$workHours->first_time_in}")
                    )
                ) {
                    $isLate = true;
                }
                ...
                $record = [
                    ...
                    'first_time_in' => $firstTimeIn
                        ...
                        : null,
                    'second_time_in' => $secondTimeIn
                        ? $secondTimeIn->timestamp
                        : null,
                    'is_late' => $isLate,
                ];
    ...
    public function getDashboardData()
    {
        ...
        if ($this->checkUser()) {
            ...
            // Get Applications
            $counter['application_count'] = ApplicationsModel::where(
                'client_id',
                $clientId
            )
                ->where('status', 'Pending')
            ...
            // Get Announcements
            $counter['announcement_count'] = AnnouncementsModel::where(
                'client_id',
                $clientId
            )
            ...
            // Get Trainings
            $counter['training_count'] = TrainingsModel::where(
                'client_id',
                $clientId
            )
                ->where('status', 'Active')
            ...
            if ($employees->count() > 0) {
                ...
                foreach ($employees as $employee) {
                    ...
                    $age =
                        ...
                        ($today->diffInMonths($birthDate) % 12) / 12 +
                        ($today->diffInDays($birthDate) % 30.42) / 365.25; // This gives the age with decimal precision
            ...
            if ($employees->count() > 0) {
                ...
                foreach ($employees as $employee) {
                    ...
                    $tenureInYears =
                        $today->diffInYears($hireDate) +
                        ($today->diffInMonths($hireDate) % 12) / 12 +
                        ($today->diffInDays($hireDate) % 30.42) / 365.25;

                    $totalTenure += $tenureInYears;
                }
                $average['tenure'] = round(
                    $totalTenure / $employees->count(),
                    1
                );
            }
            ...
            // Present Counter
            $attendance['present_count'] = AttendanceLogsModel::whereHas(
                'user',
            ...
            // On Leave Counter
            $attendance['onleave_count'] = ApplicationsModel::whereHas(
                'user',
                function ($query) use ($clientId) {
                    $query->where('client_id', $clientId);
                }
            ...
            // Branch Employees
            $branches = [];
            $rawBranches = BranchesModel::select(
                'id',
                'name',
                'acronym',
                'client_id'
            ...
            foreach ($rawBranches as $branch) {
                $employees = UsersModel::select('name')
                    ->where('user_type', "Employee")
                    ->where('employment_status', "Active")
            ...
            if ($type == 1) {
                ...
                $result = $attendances
                    ...
                    ->map(function ($logs) {
                        ...
                        if ($workHours->shift_type == "Regular") {
                            $firstTimeIn = $logs->firstWhere(
                                'action',
                                'Duty In'
                            );

                            $breakEndTime = Carbon::parse(
                                $workHours->break_end
                            )->startOfDay();
                            $firstTimeOut = $logs->last(function ($log) use (
                                $breakEndTime,
                                $workHours
                            ) {
                            ...
                            $secondTimeOut = $logs->last(function ($log) use (
                                $secondIn
                            ) {
                                return $log->action === 'Duty Out' &&
                                    Carbon::parse($log->timestamp)->gt(
                        ...
                        return [
                            ...
                            'first_time_in' => $firstTimeIn
                                ? $firstTimeIn->timestamp
                                : null,
                            'first_time_out' => $firstTimeOut
                                ? $firstTimeOut->timestamp
                                : null,
                            'second_time_in' => $secondTimeIn
                                ? $secondTimeIn->timestamp
                                : null,
                            'second_time_out' => $secondTimeOut
                                ? $secondTimeOut->timestamp
                                : null,
                ...
                $result = $attendances
                    ...
                    ->map(function ($logs) {
                        ...
                        if (
                            ...
                            $lateBy = Carbon::parse(
                                $firstAttendance->timestamp
                            )->diffInSeconds(Carbon::parse($workStart));
                            return [
                                'id' => $user->id,
                                'first_name' => $user->first_name ?? null,
                                'last_name' => $user->last_name ?? null,
                ...
                // On Leave
                $onLeaveApplications = ApplicationsModel::whereHas(
                    'user',
                    function ($query) use ($clientId) {
                        $query
                            ->where('client_id', $clientId)
    ...
    public function getEmployeeAvatars(Request $request)
    {
        ...
        if (empty($userList)) {
            return response()->json(
                ['status' => 400, 'message' => 'No user list provided'],
                400
            );
        }
        ...
        if ($listType == 1) {
            ...
            return response()->json(
                ['status' => 400, 'message' => 'Invalid user list type'],
                400
            );
        }

        $avatars = $users
            ->mapWithKeys(function ($user) use ($listType) {
                ...
                if (
                    ...
                    $avatarData['avatar'] = base64_encode(
                        Storage::disk('public')->get($user->profile_pic)
                    );
                    $avatarData['avatar_mime'] = mime_content_type(
                        storage_path('app/public/' . $user->profile_pic)
                    );
                }
